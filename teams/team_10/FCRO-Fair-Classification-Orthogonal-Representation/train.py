...
def main():
    ...
    parser.add_argument("--epoch", help="total training epochs", type=int, default=2)
    ...
    # )
    parser.add_argument(
        "--exp_path", help="path of the experiment result", type=str, default="../experiments_short"
    )
    parser.add_argument(
        "--pretrained_path",
    ...
    model_a = Network(len(["Race", "Married", "Age"]), args=args, pretrained=True).cuda()
    ...
    if args.test:
        ...
        #     mode=f"test",
        # )
        test_set = EMBED.EMBED(
            csv_path="/home/jupyter-dapamede/data_test.csv",
            # image_root_path=args.image_path,
            target_labels=args.target_label,
            sensitive_attribute=["Race", "Married", "Age"],
            transform=EMBED.data_transforms["test"],
            shuffle=False,
            mode=f"test",
        )
    ...
    train_set = EMBED.EMBED(
        csv_path="/home/jupyter-dapamede/data_train.csv",
        # image_root_path=args.image_path,
        target_labels=args.target_label,
        sensitive_attribute=["Race", "Married", "Age"],
        transform=EMBED.data_transforms["train"],
        shuffle=True,
        mode="train",
        # sheet=f"train_{args.fold}",
    )
    ...
    val_set = EMBED.EMBED(
        csv_path="/home/jupyter-dapamede/data_val.csv",
        # image_root_path=args.image_path,
        target_labels=args.target_label,
        sensitive_attribute=["Race", "Married", "Age"],
        transform=EMBED.data_transforms["val"],
        shuffle=False,
        mode=f"val",
